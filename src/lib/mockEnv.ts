...
// When building the application, import.meta.env.DEV will become false, and the code inside
// will be tree-shaken, so you will not see it in your final bundle.
if (import.meta.env.DEV) {
  ...
  // Step 2: Set a recent auth_date (current time or a few seconds earlier)
  const recentAuthDate = currentTime - 10; // Set to 10 seconds ago
  ...
  if (shouldMock) {
    const initDataRaw = new URLSearchParams([
      ...
      ['auth_date', recentAuthDate.toString()], // Update with recent auth_date
      ['start_param', 'JANESMITH2024'],
      ['chat_type', 'sender'],
